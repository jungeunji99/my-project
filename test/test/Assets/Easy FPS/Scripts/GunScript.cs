...
public class GunScript : MonoBehaviour {
	...
	void CrossHairExpansionWhenWalking(){
		...
		if(player.GetComponent<Rigidbody>().velocity.magnitude > 1 && Input.GetAxis("Fire1") == 0){//ifnot shooting
			...
			if(player.GetComponent<PlayerMovementScript>().maxSpeed < runningSpeed){ //not running
				...
				fadeout_value = Mathf.Lerp(fadeout_value, 1, Time.deltaTime * 4);
			}
			else{//running
	...
	*/
	...
	private float position_x(float var){
		return Screen.width * var / 300;
	}
	private float position_y(float var)
	{
		return Screen.height * var / 300;
	}
	...
	{
		return Screen.height * var / 200;
	}
	private Vector2 vec2(Vector2 _vec2){
		return new Vector2(Screen.width * _vec2.x / 200, Screen.height * _vec2.y / 200);
	}
	//#
	...
	*/
	void Animations(){
		...
		if(handsAnimator){
			...
			handsAnimator.SetBool("isDodge",pmS.isDodge);
    ...
    public string dodgeAnimationName = "Player_Dodge";
}
