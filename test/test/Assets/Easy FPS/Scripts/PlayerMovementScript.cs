	...
	void PlayerMovementLogic(){
		...
		else if (grounded)
		{
			...
			rb.velocity = Vector3.SmoothDamp(rb.velocity,
											  ...
											  ref slowdownV, deaccelerationSpeed);
		...
		if (Input.GetAxis ("Horizontal") != 0 || Input.GetAxis ("Vertical") != 0) {
			deaccelerationSpeed = 0.25f;
		} else {
			deaccelerationSpeed = 0.05f;
		}
	}
	/*
   ...
   IEnumerator DodgeFalse()
{
    ...
    void Dodge(){
		if(grounded && Input.GetKeyDown (KeyCode.F) && (Input.GetAxis ("Horizontal")!=0||Input.GetAxis ("Vertical")!=0) && !isDodge && !dodgeCool){
            isDodge = true;
			dodgeCool = true;
            StartCoroutine("DodgeFalse");
            ...
            rb.AddRelativeForce(dodgeDirection * dodgeForce, ForceMode.VelocityChange);
           

        }
		else if(grounded&&Input.GetKeyDown(KeyCode.F)&& !isDodge && !dodgeCool)
        {
            dodgeCool = true;
            isDodge = true;
				StartCoroutine("DodgeFalse");
            StartCoroutine("DodgeCooldown");
            rb.velocity = Vector3.zero;
            rb.AddRelativeForce(Vector3.forward*dodgeForce,ForceMode.VelocityChange);
